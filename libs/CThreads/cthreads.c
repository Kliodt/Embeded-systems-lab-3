...
int cthreads_thread_create(struct cthreads_thread *thread,
    ...
    if (attr)
        thread->wThread = CreateThread(
            NULL, attr->stacksize ? attr->stacksize : 0,
            __cthreads_winthreads_function_wrapper, args,
            attr->dwCreationFlags ? (DWORD)attr->dwCreationFlags : 0, NULL);
    else
        thread->wThread = CreateThread(
            NULL, 0, __cthreads_winthreads_function_wrapper, args, 0, NULL);
...
#ifdef _WIN32
    return SleepConditionVariableCS(&cond->wCond, &mutex->wMutex, INFINITE) ==
           0;
#else
...
#ifdef _WIN32
    return SleepConditionVariableCS(&cond->wCond, &mutex->wMutex, (DWORD)ms) ==
           0;
#else
    struct timespec ts;

 ...
 *
 * @param thread Pointer to the thread structure to be filled with the new
 * thread information.
 * @param attr Pointer to the thread attributes. Set it to NULL for default
 * attributes.
 ...
 */
int cthreads_thread_create(struct cthreads_thread *thread,
                           struct cthreads_thread_attr *attr,
 ...
 */
int cthreads_thread_equal(struct cthreads_thread thread1,
 ...
 * @param attr Pointer to the mutex attributes. Set it to NULL for default
 * attributes. Only available if CTHREADS_MUTEX_ATTR is defined.
 * @return 0 on success, non-zero error code on failure.
 */
#ifdef CTHREADS_MUTEX_ATTR
int cthreads_mutex_init(struct cthreads_mutex *mutex,
                        struct cthreads_mutex_attr *attr);
#else
 ...
 * @param attr Pointer to the condition variable attributes. Set it to NULL for
 * default attributes. Only available if CTHREADS_COND_ATTR is defined.
 * @return 0 on success, non-zero error code on failure.
 */
#ifdef CTHREADS_COND_ATTR
int cthreads_cond_init(struct cthreads_cond *cond,
                       struct cthreads_cond_attr *attr);
#else
 ...
 * @return 0 on success, non-zero error code on failure.
 */
int cthreads_cond_wait(struct cthreads_cond *cond,
 ...
 * @return 0 on success, non-zero error code on failure.
 */
int cthreads_cond_timedwait(struct cthreads_cond *cond,
                            struct cthreads_mutex *mutex, unsigned int ms);

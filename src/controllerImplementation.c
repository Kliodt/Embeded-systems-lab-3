...
printScreen (void)
{
  ...
  int iter = 0;
  for (int i = 0; i < SCREEN_WIDTH_PX * 2 + 2; i++)
    {
      str[iter++] = '-';
    }
  str[iter++] = '\n';

  // правый и левый край экрана
...
void
drawLabel (int x, int y, char str[])
{
...
void
drawSquare (int x, int y, int sizePx)
{
  // todo: check screen boundaries
  // ! this is debug code
  int new_y = y % (SCREEN_HEIGHT_PX * 2 + 2);
  int new_x = x % (SCREEN_WIDTH_PX * 2 + 2);

  for (int i = new_y; i < new_y + sizePx; i++)
    {
      for (int j = new_x; j < new_x + sizePx; j++)
        {
...
void
drawMatrix (bool ** matrix, int x, int y, int widthPx, int heightPx)
{
